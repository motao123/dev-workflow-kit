...
| --- | --- | --- | --- |
| 需求不清、范围模糊 | `planning` | 任务拆解、风险、验收标准、验证方案 | Before `trellis-lightweight-tdd` |
...
1. `planning`
2. `architecture-refactor-guidance` when structure is the real problem
3. `trellis-lightweight-tdd`
4. `systematic-debugging` when needed
5. `incident-observability-triage` when alerts, logs, or metrics drive the investigation
6. `fix-ci` when automation disagrees
7. `review-address-comments`
8. `browser-verification` / `security-review` / `dependency-compliance-audit` when applicable
9. `ship-readiness`
10. `docs-writer`
